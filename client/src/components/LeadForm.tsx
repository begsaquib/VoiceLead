...
const WEBHOOK_URL =
  "https://flow.wecommit.ai/webhook/91337cef-2e74-4cb0-b970-1f6af5a8956d";
const DIRECTUS_URL =
  (typeof window !== "undefined" && (window as any).VITE_DIRECTUS_URL) ??
  "http://localhost:8055";
const DIRECTUS_TOKEN =
  typeof window !== "undefined"
    ? (window as any).VITE_DIRECTUS_TOKEN
    : undefined;
const BOOTH_ID =
  typeof window !== "undefined" ? (window as any).VITE_BOOTH_ID : undefined;
...
export function LeadForm({ onSuccess, onCancel }: LeadFormProps) {
  ...
  if (extractedData && formData) {
    return (
      <div className="bg-gradient-to-br from-slate-950 to-[#0a0c10] border border-white/10 rounded-[48px] p-12 shadow-2xl shadow-black/50 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/5 to-transparent opacity-0 hover:opacity-100 transition-opacity pointer-events-none" />
        ...
        <div className="relative z-10 space-y-8">
          ...
          <div className="space-y-4">
            {/* Name Input */}
            <div className="group p-4 bg-white/5 border border-white/10 rounded-2xl hover:border-indigo-500/50 transition-all duration-300 overflow-hidden relative">
            ...
            <div className="group p-4 bg-white/5 border border-white/10 rounded-2xl hover:border-indigo-500/50 transition-all duration-300 overflow-hidden relative">
              ...
              <div className="relative z-10 space-y-2">
                ...
                <textarea
                  ...
                  onChange={(e) => updateField("interest", e.target.value)}
                  rows={3}
                  className="w-full bg-transparent text-white placeholder-slate-600 text-lg font-semibold focus:outline-none resize-none"
                />
              </div>
            </div>
          </div>
          ...
          {error && (
            <div className="flex gap-3 p-4 bg-red-950/40 border border-red-500/30 rounded-2xl">
          ...
          <div className="flex gap-4 pt-4">
            <button
              onClick={onCancel}
              className="flex-1 px-6 py-3 bg-white/5 border border-white/10 rounded-2xl text-slate-300 hover:text-white hover:bg-white/10 hover:border-white/20 font-bold transition-all duration-300 uppercase tracking-wider text-sm"
            >
              Discard
            </button>
            <button
              onClick={handleSave}
              className="flex-1 px-6 py-3 bg-gradient-to-r from-indigo-600 to-indigo-500 text-white rounded-2xl font-bold shadow-lg shadow-indigo-600/20 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 uppercase tracking-wider text-sm"
            >
              Save Lead
            </button>
          </div>
  ...
  if (isProcessing) {
    return (
      <div className="bg-gradient-to-br from-slate-950 to-[#0a0c10] border border-white/10 rounded-[48px] p-20 text-center shadow-2xl shadow-black/50 relative overflow-hidden">
  ...
  return (
    <div className="bg-gradient-to-br from-slate-950 to-[#0a0c10] border border-white/10 rounded-[48px] p-20 text-center shadow-2xl shadow-black/50 relative overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/5 to-transparent opacity-0 hover:opacity-100 transition-opacity" />

      {error && (
        <div className="mb-8 flex gap-3 p-4 bg-red-950/40 border border-red-500/30 rounded-2xl">
          <XCircle className="w-5 h-5 text-red-400 flex-shrink-0 mt-0.5" />
          <p className="text-red-300 font-medium text-sm">{error}</p>
        </div>
      )}
      ...
      <div className="relative z-10 space-y-12">
        <div className="relative">
          {isRecording ? (
            <>
              ...
              <button
                onClick={stopRecording}
                className="relative w-40 h-40 bg-gradient-to-br from-red-600 to-red-700 rounded-full flex items-center justify-center text-white shadow-2xl shadow-red-600/40 hover:scale-110 active:scale-95 transition-all mx-auto group"
              >
            ...
            <>
              ...
              <button
                onClick={startRecording}
                className="relative w-40 h-40 bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-full flex items-center justify-center text-white shadow-2xl shadow-indigo-600/40 hover:scale-110 active:scale-95 transition-all mx-auto group"
              >
                <Mic className="w-16 h-16 group-hover:animate-bounce" />
              </button>
            </>
          )}
        </div>
        ...
        {isRecording ? (
          <>
            <div className="inline-flex items-center gap-3 px-6 py-3 bg-red-950/40 border border-red-500/30 rounded-full mx-auto">
              <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse" />
              <span className="text-red-300 font-black text-2xl font-mono">
                {formatTime(recordingTime)}
              </span>
            </div>
            <div className="space-y-4">
            ...
            <button
              onClick={stopRecording}
              className="px-12 py-4 bg-white text-black rounded-2xl font-black uppercase tracking-widest hover:bg-slate-100 transition-all text-sm mx-auto block"
            >
              Stop & Process
            </button>
          </>
          ...
          <>
            ...
            <button
              onClick={onCancel}
              className="text-slate-400 hover:text-white font-bold transition-colors uppercase tracking-wider text-sm"
            >
              Cancel
            </button>
          </>
